...
const BubbleSort: React.FC = () => {
  ...
  const bubbleSort = useCallback((arr: number[]): Step[] => {
    ...
    steps.push({
      ...
      j: -1,
      comparing: false,
      description: `Bắt đầu thuật toán sắp xếp nổi bọt ${isAscending ? "tăng dần" : "giảm dần"}`,
    });
    ...
    for (let i = 0; i < sortedArr.length - 1; i++) {
      for (let j = 0; j < sortedArr.length - i - 1; j++) {
        steps.push({
          ...
          j,
          comparing: true,
          description: `So sánh A[${j}] = ${sortedArr[j]} với A[${j + 1}] = ${sortedArr[j + 1]}`,
        });
    ...
    steps.push({
      ...
      j: -1,
      comparing: false,
      description: `Thuật toán sắp xếp nổi bọt ${isAscending ? "tăng dần" : "giảm dần"} hoàn tất`,
    });
                  ...
                  <div
                    ...
                    className={`w-12 h-12 rounded-full flex items-center justify-center mr-2 mb-2 transition-all duration-500
                      ${
                        index === currentStepData.j
                          ...
                          ? "bg-red-300"
                          : index >= currentStepData.array.length - currentStepData.i
                          ? "bg-gray-300"
                          : "bg-purple-400"
                      }
                      ${
                        currentStepData.comparing &&
                        (index === currentStepData.j || index === currentStepData.j + 1)
                    ...
                    style={{ 
                      transform: currentStepData.animatingIndices && 
                        ...
                        ? `translateX(${index === currentStepData.animatingIndices.from ? '3rem' : '-3rem'})`
                        : 'none'
                    }}
                  >
                    {num}
