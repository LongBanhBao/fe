...
const TKTT: React.FC = () => {
  // Đây là một file TypeScript React (.tsx) vì:
  // 1. Sử dụng TypeScript interfaces (Step, CodeEditorProps)
  // 2. Sử dụng React.FC type annotation
  // 3. Có type annotations cho state và props
  // 4. Import React và các type từ React
  // 5. Sử dụng JSX syntax
  // 6. Có đuôi file là .tsx
...
`;
  ...
  useEffect(() => {
    if (isRunning && currentStep < searchSteps.length) {
      ...
      setIsRunning(false);
      setResult(searchSteps[searchSteps.length - 1]?.index ?? -1);
    }
