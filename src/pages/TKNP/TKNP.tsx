...
const TKNP: React.FC = () => {
  ...
  const isSorted = (arr: number[]): boolean => {
    ...
    const isAscending = arr.every((val, i, arr) => i === 0 || val >= arr[i - 1]);
    // Kiểm tra dãy số có được sắp xếp giảm dần
    const isDescending = arr.every((val, i, arr) => i === 0 || val <= arr[i - 1]);
          ...
          <div className="step-container overflow-hidden">
            ...
            {currentStepData.comparing && (
              <div className="flex mb-4 items-center">
                ...
                <div className={`w-12 h-12 rounded-full flex items-center justify-center mr-2 bg-yellow-300`}>
                  {currentStepData.value}
                </div>
                <p className="mx-2 text-2xl">
                  {currentStepData.isMatch ? "=" : currentStepData.value! < k ? "<" : ">"}
                </p>
                <p className="mr-2">K = </p>
                <div className={`w-12 h-12 rounded-full flex items-center justify-center mr-2 bg-blue-300`}>
                  {k}
                </div>
              </div>
            )}
                ...
                <div className="whitespace-pre-line">
                  ...
                  (currentStepData.isMatch 
                    ...
                    : currentStepData.value! < k 
                      ? `Vì A[${currentStepData.mid}] < K => i + 1 = ${currentStepData.mid + 1}`
                      : `Vì A[${currentStepData.mid}] > K => i - 1 = ${currentStepData.mid - 1}`)}
                </div>
              )}
            </div>
